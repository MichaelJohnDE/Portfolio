  ...
  return (
    <section id="about" className="pt-24 md:pt-32 pb-section-padding-mobile md:pb-section-padding-desktop px-[5%] md:px-[6%] lg:px-[8%] w-full overflow-hidden flex flex-col justify-center min-h-[70vh]">
      <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12 lg:gap-16 justify-between w-full text-center md:text-left">
        ...
        >
          <h1 className="font-display text-4xl sm:text-5xl md:text-display text-on-surface mb-4 md:mb-6" style={{ textWrap: 'balance' }}>
            Hi, I'm <br className="block sm:hidden" /><span className="text-primary">{profile.firstName} {profile.lastName}</span>.<br />
            {/* Mobile role */}
            <span className="text-2xl sm:text-3xl mt-2 block md:hidden">{profile.roles[0]}</span>
            {/* Desktop role */}
            <span className="hidden md:inline">{profile.roles[0]}</span>
          </h1>
        ...
        >
          <div className="flex flex-col gap-4 w-full sm:w-80 md:w-96">
            ...
            {profile.socials.map((social: any, index: number) => {
              const Icon = social.icon === 'github' ? Github : social.icon === 'linkedin' ? Linkedin : social.icon === 'mail' ? Mail : LinkIcon;
