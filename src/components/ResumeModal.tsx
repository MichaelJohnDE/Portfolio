...
const ResumeModal = ({ isOpen, onClose, profile }: { isOpen: boolean, onClose: () => void, profile: any }) => {
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {/* Modal Body */}
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#0a0a0c] min-h-[60vh] md:min-h-[80vh]">
              <div className="flex justify-center w-full h-full">
                <iframe 
                  src={`${profile.resumeUrl}#view=FitH`}
                  title="Resume Preview"
                  className="w-full h-full min-h-[60vh] md:min-h-[80vh] rounded-sm shadow-2xl shadow-black block bg-white"
                />
              </div>
            </div>
          </motion.div>
